...
def plot_dust_temp_amiy(username, diffuse, errorcheck = "errorcheck", includePAHS = "includePAHS"):
    ...
    sa.cd_amiy('/Users/' + username + '/dust_modelling/mocassin.2.02.70/')
    ...
    if diffuse:
        sizefile='input/mrnsmall.dat'
    elif (includePAHS == "includePAHS"):
    ...
    dustprop = [[[MyStructDustProp(1, nspecies + 1, nsizes + 1) for col in range(nx)] for row in range(ny)] for x in
                range(nz)]
            ...
            for k in range(nx - 1):
                ...
                for l in range(nsizes - 1):
                    for m in range(nspecies - 1):
                        ...
                        if (math.sqrt(r[i, j, k] ^ 2 - x[i] ^ 2) / r[i, j, k] > 0.707107 and convergence[i, j, k].n[
                            1] == 1):
                            t1[ii] = temp[i, j, k]
        if (convergence[i, j, k].n[1] == 1):
            if (math.sqrt(r[i, j, k] ^ 2 - x[i] ^ 2) / r[i, j, k] > 0.707107):
...
#the dust shell along the x - axis(to account for torus models!)
    ...
    plt.plot(plotRout, plotTemp_2, marker = 'g+')
...
#stop
    ...
    plt.ylabel('Temp (k)')
    plt.axis([min(dustsizes.radius), max(dustsizes.radius),ymin - 15, ymax + 15])
    ...
    if (includePAHS == "includePAHS"):
        ...
        plt.plot(dustsizes.radius, temp[elements - 2, 0, 0], marker = 's')
        plt.title('Temp vs size')
        plt.xlabel('size in microns')
        plt.ylabel('Temp (k)')
